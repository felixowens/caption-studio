...
type CaptioningService interface {
	...
	GenerateEditCaption(imageABase64, imageBBase64, systemPrompt string) (string, error)
}
...
func (g *GeminiService) GenerateEditCaption(imageABase64, imageBBase64, systemPrompt string) (string, error) {
	if g.APIKey == "" {
		return "", fmt.Errorf("Gemini API key not configured")
	}

	// Default system prompt for edit captions if none provided
	if systemPrompt == "" {
		...
		Contents: []GeminiContent{
			{
				Parts: []GeminiPart{
					...
					{
						InlineData: &GeminiInlineData{
							MimeType: getMimeType(imageABase64),
							Data:     imageABase64,
						},
					},
					{
						InlineData: &GeminiInlineData{
							MimeType: getMimeType(imageBBase64),
							Data:     imageBBase64,
						},
					},
				},
	...
	if len(geminiResponse.Candidates) == 0 || len(geminiResponse.Candidates[0].Content.Parts) == 0 {
		logger.Error("No edit caption generated by Gemini API", "response", string(responseBody))
		return "", fmt.Errorf("no edit caption generated by Gemini API")
	}
